...
>>>>>>> b4de11e535f7eded47f763242f3d62be24cb62ad
poc_level_1 <- poc %>% 
  select(Starting) %>% 
...
## Inputs
max_level <- max(poc_levels$num)

Task_Names <- poc_levels$Segment

task_duration <- poc_join %>%
  select(Starting, Duration) %>% 
  distinct_all()

task_duration2 <- as.integer(append(task_duration$Duration,0))
...
## Schedule Creation
sch_dynamic <- sch_new() %>% 
  sch_title("Raymonde Corp Move to Mexico") %>% 
  sch_add_activities(
    id = 1:max_level
    ,name = Task_Names
...
sch_duration(sch_dynamic)

sch_activities(sch_dynamic)

sch_relations(sch_dynamic)
